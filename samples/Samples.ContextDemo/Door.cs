...
internal sealed partial class Door
{
    ...
    internal static State CreateInitialState() => Opened.Instance;

    private void WriteLine(string value, [CallerMemberName] string callerName = "") =>
        Out.WriteLine($"[{nameof(Door)}.{callerName}] {value}");

    private void OnExitingOpened(Event ev, Opened currentState, State newState) =>
        WriteLine(
    ...
    private void OnEnteredOpened(Event ev, Opened currentState, State oldState) =>
        WriteLine(
            $"{nameof(ev)}: {ev}, {nameof(currentState)}: {currentState.GetType().Name}, {nameof(oldState)}: {oldState.GetType().Name}");

    private void OnExitingClosed(Event ev, Closed currentState, State newState) =>
        WriteLine(
            $"{nameof(ev)}: {ev}, {nameof(currentState)}: {currentState.GetType().Name}, {nameof(newState)}: {newState.GetType().Name}");
    ...
    private void OnEnteredClosed(Event ev, Closed currentState, State oldState) =>
        WriteLine(
            $"{nameof(ev)}: {ev}, {nameof(currentState)}: {currentState.GetType().Name}, {nameof(oldState)}: {oldState.GetType().Name}");

    private void OnExitingLocked(Event ev, Locked currentState, State newState) =>
        WriteLine(
            $"{nameof(ev)}: {ev}, {nameof(currentState)}: {currentState.GetType().Name}, {nameof(newState)}: {newState.GetType().Name}");
    ...
    private void OnEnteredLocked(Event ev, Locked currentState, State oldState) =>
        WriteLine(
            $"{nameof(ev)}: {ev}, {nameof(currentState)}: {currentState.GetType().Name}, {nameof(oldState)}: {oldState.GetType().Name}");
}
